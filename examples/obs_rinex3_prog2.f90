...
!> (a timestamp).
!>
!> Input:  The RINEX FILE_NAME constant, which can be modified in the
!>         code.
...
!>    Allocate one memory buffer for processing all epoch records.
!> 5. Reading the first epoch record.
!> 6. Get the satellite system index and observation index.
!> 7. Print the selected data.
!>
!> Note on implementation: The file is read line by line, so these
...
program obs_rinex3_prog2
  ...
  use obs_rinex3, only: ip, wp, &
       header_t, epoch_record_t, &
       get_sys_index, &
       get_obs_index, &
       read_header, &
       allocate_epoch_record_buffer, &
  ...
  associate (A => h%sys_num_obs_types)
    ...
    ! 4. Mandatory preparing for reading epoch recrods
    call allocate_epoch_record_buffer(A, epr)
    ...
    if (i_G > 0 .and. i_G <= size(B)) then
       ...
       if (i_L1C > 0) then
          print '(A, I4.4,4(1X,I2.2),F11.7)', &
               "Epoch: ", epr%header%year, epr%header%month, epr%header%day, &
                          epr%header%hour, epr%header%min, epr%header%sec

          do i_prn = 1, B(i_G)%n_prns
             print '(A,I2.2,A,F14.3)', &
                  "  Satellite: G", B(i_G)%prns(i_prn), &
                  " | L1C: ", B(i_G)%sys_sats_obs(i_L1C, i_prn)%val
          end do
       else
