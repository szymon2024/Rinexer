...
!> files.
!>
!> Note: The current version does not support handling special events
!> and comments within the observation data.
!>
!> HOW TO USE IT IN YOUR FORTRAN PROGRAM:
!>
!> 1. Copy the `obs_rinex3.f90` file to your project directory.
!> 2. Add a usage declaration in your code:
!>    program my_program
!>       use obs_rinex3
!> 3. Compile your program by providing both source files at the same
!>    time:
!>    on Linux
!>    gfortran obs_rinex3.f90 my_program.f90 -o my_program
!>
!>    on Windows
!>    gfortran obs_rinex3.f90 my_program.f90 -o my_program.exe
!>
!> Note on terminology: An epoch represents a specific moment in time
!> (timestamp).
!>
!> Note on implementation: The RINEX file is read line by line.
module obs_rinex3
  ...
  public :: wp, ip
  public :: MAX_SYS
  public :: header_t, epoch_record_t
  public :: rinex_version_type_t
  public :: read_header
  public :: allocate_epoch_record_buffer
...
contains
  ...
  subroutine validate_header(h, err_msg)
    ...
    if (prev_err_msg /= "") &
         write(err_msg, '(3A)') &
         "Header Control:", new_line('a'), trim(prev_err_msg)
  ...
  subroutine allocate_epoch_record_buffer(A, r)
    type(sys_num_obs_types_t), intent(in)    :: A(:)
  ...
  end subroutine allocate_epoch_record_buffer
  ...
  subroutine read_next_epoch_record(unit, A, r, ios, err_msg)
    ...
    read(line, '(1X,1X,I4,4(1X,I2.2),F11.7,2X,I1,I3,6X,F15.12)', iostat=ios) &
         r%header%year, r%header%month, r%header%day, &
         r%header%hour, r%header%min, r%header%sec, &
         r%header%flag, r%header%n_sats, r%header%rcv_clk_offset
    ...
    associate (B => r%sats_obs)
      ...
      ! epoch
      sat_loop: do s = 1, r%header%n_sats
         read(unit, '(A)', iostat=ios) line
         if (ios /= 0) then
            if (ios == iostat_end) then
               return
         ...
         if (prev_err_msg /= "") then
            write(err_msg, '(A, I4, 4(1X,I2.2), F11.7, 2A)') &
                 "Epoch: ", r%header%year, r%header%month, r%header%day, &
                 r%header%hour, r%header%min, r%header%sec, &
  ...
  subroutine write_epoch_header(unit, r)
    ...
    if (r%header%rcv_clk_offset == 0.0_wp) then
       write(unit, '(A1, 1X, I4, 4(1X,I2.2), F11.7, 2X, I1, I3)') &
            '>', r%header%year, r%header%month, r%header%day, &
            r%header%hour, r%header%min, r%header%sec, &
            r%header%flag, r%header%n_sats
    else
