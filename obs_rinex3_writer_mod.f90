...
contains
  ...
  subroutine write_epoch_obs(unit, A, B)
    integer(ip),               intent(in) :: unit
    type(sys_num_obs_types_t), intent(in) :: A(:)
    type(sys_obs_by_sat_t),    intent(in) :: B(:)

    character(1) :: sat_sys
    integer(ip)  :: i, j, n_obs, i_obs, last_present_obs, k

    on_sys: do i = 1, size(B)
       if (allocated(B(i)%sys_obs_by_sat)) then
          ...
          on_sat: do j = 1, B(i)%n_prns
             ...
             last_present_obs = 0
             do i_obs = n_obs, 1, -1
                if (B(i)%sys_obs_by_sat(i_obs,j)%val /= 0.0_wp) then
             ...
             do k = 1, last_present_obs
                if (k == last_present_obs) then
                   if (B(i)%sys_obs_by_sat(k,j)%lli == 0 .and. B(i)%sys_obs_by_sat(k,j)%ssi == 0) then
                      write(unit, '(A14)', advance='NO') f14_3(B(i)%sys_obs_by_sat(k,j)%val)
                   else if (B(i)%sys_obs_by_sat(k,j)%ssi == 0) then
                      write(unit, '(A14, I1.0)', advance='NO') &
                           f14_3(B(i)%sys_obs_by_sat(k,j)%val), B(i)%sys_obs_by_sat(k,j)%lli
                   else
                      write(unit, '(A14, I1.0, I1.0)', advance='NO') &
                           f14_3(B(i)%sys_obs_by_sat(k,j)%val), &
                           B(i)%sys_obs_by_sat(k,j)%lli, &
                           B(i)%sys_obs_by_sat(k,j)%ssi
                   end if
                else
                   write(unit, '(A14, I1.0, I1.0)', advance='NO') &
                        f14_3(B(i)%sys_obs_by_sat(k,j)%val), &
  ...
  subroutine write_epoch_record(unit, A, r)
    ...
    type(epoch_record_t),      intent(in) :: r
      ...
      call write_epoch_obs(unit, A, r%obs_by_sys)
